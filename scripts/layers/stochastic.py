...
import layers.stochastic

from utils import reduce_to_batch, reduce_to_latent
from variational import kl_divergence_mc, dirichlet_kl_divergence
...
def get_stochastic(name: str):
    ...
    try:
        klass = getattr(layers.stochastic, name)
    except KeyError:
        raise KeyError(f"No StochasticModule of name {name} is defined.")
    return klass


STOCHASTIC_DATA_FIELDS = ["params", "z", "dist", "mean", "variance", "use_mode", "forced_latent"]
...
class GaussianStochasticModule(StochasticModule):
    ...
    def loss(self, q_data: StochasticData, p_data: StochasticData) -> LossData:
        ...
        # kl_elementwise, q_logprob, p_logprob = dirichlet_kl_divergence(q_data.z, q_data.params, p_data.params, q_data.dist, p_data.dist)
...
class DirichletStochasticModule(StochasticModule):
    """Base module for StochasticModules with a multivariate Dirichlet distribution.
    ...
    def define_prior(self, *shape):
        """Define the Dir with sparse-inducing parameter

        Since we apply Softplus to the scale paramteer we must do the inverse for the initial value.
        """
        prior = torch.full((shape), 0.99) # sparse prior
                        
        if self.learn_prior:
            self.prior_logits = nn.Parameter(prior)
            # self.prior_logits = torch.clamp(prior_logits, max=0.9) # learn - but restrict
        else:
            self.register_buffer("prior_logits", prior)

    @property
    def prior(self):
        """Return the prior distribution without a batch dimension"""
        alpha = self.prior_logits.double()
        return D.Dirichlet(alpha, validate_args=True)

    def concentration_from_logits(self, logits: Tensor) -> Tensor:
        return F.softplus(logits) + self.min_scale
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Define distribution
        if x is None:
            alpha = self.concentration_from_logits(self.prior_logits.expand(n_prior_samples, *self.prior_logits.shape)).double()
        else:
            alpha = self.compute_params(x, inference).double()

        dist = D.Dirichlet(alpha, validate_args=True)
    ...
    def loss(self, q_data: StochasticData, p_data: StochasticData) -> LossData:
        ...
        kl_elementwise, q_logprob, p_logprob = kl_divergence_mc(q_data.z, q_data.params, p_data.params)
...
class DirichletConv2d(DirichletStochasticModule):
    """A Dirichlet stochastic layer parametrized by a convolution."""
...
class DirichletDense(DirichletStochasticModule):
    """A Normal stochastic layer parametrized by a dense layer."""
    ...
    def __init__(
        ...
        if top:
            self.define_prior(self.latent_features) #self.latent_features)
